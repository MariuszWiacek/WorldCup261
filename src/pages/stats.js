  ...
  useEffect(() => {
    ...
    Object.keys(submittedData).forEach(user => {
      ...
      Object.entries(bets).forEach(([matchId, bet]) => {
        ...
        // TEAM TRACKING
        // --------------------
        const home = bet.home || "Unknown";
        const away = bet.away || "Unknown";
      ...
      // ⚽ TEAM FILTER (avoid fake stats, min 3 games played)
      ...
      <Row>
        <Col xs={12}>
          <div style={{ marginTop: '10px', marginBottom: '20px', color: '#FFD700', textAlign: 'center' }}>
            ...
            <p style={{ color: '#aaa' }}>Detailed breakdown of player forecasting tendencies</p>
      ...
      <Row className="justify-content-center">
        <Col xs={12} md={8} lg={6}>
          ...
          {profiles.map((p, idx) => (
            ...
            >
              {/* Header: User & OVR Rating */}
              <div style={{ display: 'flex', justifyContent: 'between', alignItems: 'center', borderBottom: '1px solid #333', paddingBottom: '10px', marginBottom: '15px' }}>
              ...
              <Row style={{ marginBottom: '15px' }}>
                <Col xs={6}>
                  <div style={{ background: '#161616', padding: '10px', borderRadius: '8px', textAlign: 'center' }}>
                    <div style={{ fontSize: '0.8rem', color: '#aaa' }}>🔮 OUTCOME ACCURACY</div>
                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#4caf50' }}>{pct(p.outcomeRate)}</div>
                    <div style={{ fontSize: '0.75rem', color: '#666' }}>({p.outcomeCorrect}/{p.outcomeTotal} matches)</div>
                  </div>
                </Col>
                <Col xs={6}>
                  <div style={{ background: '#161616', padding: '10px', borderRadius: '8px', textAlign: 'center' }}>
                    <div style={{ fontSize: '0.8rem', color: '#aaa' }}>🎯 EXACT SCORE HITS</div>
                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#2196f3' }}>{pct(p.scoreRate)}</div>
                    <div style={{ fontSize: '0.75rem', color: '#666' }}>({p.scoreCorrect} exact hits)</div>
                  </div>
                </Col>
              </Row>

              {/* Team Dynamics */}
              <div style={{ fontSize: '0.9rem', marginBottom: '15px', lineBottom: '1px solid #333' }}>
                <div style={{ margin: '4px 0', color: '#ccc' }}>
                  <span style={{ color: '#4caf50' }}>⚽ Szczęśliwe zespoły:</span> {p.bestPointTeams.join(', ') || 'Brak danych'}
                </div>
                <div style={{ margin: '4px 0', color: '#ccc' }}>
                  <span style={{ color: '#f44336' }}>💔 Pechowe zespoły:</span> {p.worstPointTeams.join(', ') || 'Brak danych'}
                </div>
              </div>
              ...
              <div style={{ 
                ...
                {p.style === "Taktyczny typer" && "Świetnie przewidujesz, kto wygra lub zremisuje, ale brakuje Ci precyzji snajpera do trafiania dokładnego rezultatu Bramkowego. Bezpieczna, systematyczna taktyka!"}
                {p.style === "Łowca wyników" && "Prawdziwy hazardzista premium! Masz nosa do idealnych wyników (np. 2:1, 0:0). Idziesz po całą pulę i nie zadowalasz się półśrodkami."}
                {p.style === "Zrównoważony analityk" && "Klasyczny styl menedżerski. Potrafisz wypośrodkować ryzyko między czystym trafem 1X2 a dokładnym wynikiem. Solidny gracz turniejowy."}
                {p.style === "Chaotyczny typer" && "Zupełnie nieprzewidywalna forma. Twoje typy potrafią zaskoczyć zarówno algorytm, jak i samych piłkarzy. Potrzebujesz więcej stabilizacji!"}
              </div>

              {/* Meta information bottom footer */}
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '15px', paddingTop: '8px', borderTop: '1px dashed #333', fontSize: '0.75rem', color: '#777' }}>
                <span>Aktywne typy: {p.outcomeTotal}</span>
                <span>Puste kupony: {p.emptyBets}</span>
              </div>
